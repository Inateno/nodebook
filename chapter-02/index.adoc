...
- *Windows* : PowerShell, Console, Git Bash.
...
Voici un site utilitaire se basant sur les métriques citées précédemment. Il vous permet en un clin d'oeil d'avoir un comparatif entre plusieurs modules souvent équivalent: [URL]#https://npms.io#

Attention tout de même. Un module ayant fait ses preuves et n'ayant plus un grand besoin d'évolution se verra attribuer une mauvaise note sur le critère "maintenance", de même un module fiable et maintenu peux ne pas être populaire sans que cela ne gage d'un mauvais module
